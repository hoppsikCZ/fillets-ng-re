...
bool Unit::canDrive() const {
    return m_model->isAlive() && !m_model->isLost() && !m_model->isBusy();
}
//-----------------------------------------------------------------
/**
...
bool Unit::willMove() const {
    return m_model->isAlive() && !m_model->isLost();
}
//-----------------------------------------------------------------
/**
 ...
 */
char Unit::driveBorrowed(const InputProvider *input, const KeyControl &buttons) {
    if (canDrive()) {
 ...
 */
char Unit::mySymbol(SDL_Keycode key) const {
...
bool Unit::isMoving() const {
    ...
    if (canDrive()) {
        ...
        result = action == "move_left" || action == "move_right" || action == "move_up" || action == "move_down" ||
                 action == "turn";
    }
    return result;
}
...
//-----------------------------------------------------------------
bool Unit::isDrivenBy(char symbol) const {
    return m_symbols.getLeft() == symbol || m_symbols.getRight() == symbol || m_symbols.getUp() == symbol ||
